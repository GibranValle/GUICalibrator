from customtkinter import CTkFrame, CTkButton, CTkLabel, CTkOptionMenu, CTkSwitch, StringVar
...
class Manual:
    ...
    def only_start(self):
        ...
        gui.stop_button_man.configure(state='disabled')

    def not_only_start(self):
        gui = self.gui
        gui.start_button_man.configure(state='disabled')
        gui.pause_button_man.configure(state='normal')
        gui.stop_button_man.configure(state='normal')
    ...
    def create_manual_frame(self):
        ...
        gui.label_manual.pack(pady=(5, 0), padx=20)
        ...
        gui.start_button_man = CTkButton(gui.frame_buttons, command=lambda: self.pushed('start'), font=gui.font_text,
                                         width=WIDTH_3, text=PLAY, text_color_disabled=DISABLED_COLOR)
        gui.start_button_man.grid(row=0, column=1)
        gui.pause_button_man = CTkButton(gui.frame_buttons, command=lambda: self.pushed('pause'), font=gui.font_text,
                                         width=WIDTH_3,
                                         text=PAUSE, state='disabled', text_color_disabled=DISABLED_COLOR,
                                         fg_color=OK_COLOR, hover_color=OK_COLOR_HOVER)
        gui.pause_button_man.grid(row=0, column=2, padx=10)
        gui.stop_button_man = CTkButton(gui.frame_buttons, command=lambda: self.pushed('stop'), font=gui.font_text,
                                        width=WIDTH_3,
                                        text=STOP, state='disabled', text_color_disabled=DISABLED_COLOR,
                                        fg_color=ERR_COLOR, hover_color=ERR_COLOR_HOVER)
        gui.stop_button_man.grid(row=0, column=3)
        gui.label_output1_man = CTkLabel(gui.frame_manual, text='M1:', font=gui.font_output, anchor='w', height=20)
        gui.label_output1_man.pack(padx=30, fill='x')
        gui.label_output2_man = CTkLabel(gui.frame_manual, text='M2:', font=gui.font_output, anchor='w', height=20)
        gui.label_output2_man.pack(padx=30, fill='x')
        gui.main_menu = CTkButton(gui.frame_manual, command=gui.generic.back2main, font=gui.font_text, text='Main menu',
                                  fg_color=ERR_COLOR, hover_color=ERR_COLOR_HOVER)
        gui.main_menu.pack(pady=(5, 0), padx=10)
        ...
        gui.switch_auto = CTkSwitch(gui.frame_manual, text='Autoclick ok', font=gui.font_text,
                                    variable=switch_var, onvalue="on", offvalue="off",
                                    command=self.handleSwitch)
        gui.switch_auto.pack(pady=5, padx=10)
